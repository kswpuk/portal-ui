...
interface StatusBoxProps {
  /**
   * Title to display in the status box
   */
  title: string

  /**
   * Text to display by default (e.g. if no reference has been received)
   */
  defaultText: string

  /**
   * Text to display if a reference has been submitted, but not yet accepted
   */
  submittedText: string

  /**
   * Text to display if a reference has been submitted and accepted
   */
  acceptedText: string
...
export default function ApplicationStatus(){
  ...
  const { register, handleSubmit, formState: { errors }, reset } = useForm<ApplicationStatusInputs>();
  const [query, setQuery] = useState<ApplicationStatusInputs | null>(null)

  const { data: status, error, isLoading } = useGetStatusQuery(query || {membershipNumber: "", dateOfBirth: ""}, {skip: !query})

  const onSubmit = (data: ApplicationStatusInputs) => {
    setQuery(data)
  }
  ...
  if(status && query) {
    ...
    return <>
      <Typography variant="h4" gutterBottom>Application Status for {status.membershipNumber} - KSWP Portal</Typography>
