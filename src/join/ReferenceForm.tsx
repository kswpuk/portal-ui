...
export interface ReferenceFormRatingInputs {
  maturity: number
...
export interface ReferenceFormInputs extends ReferenceFormRatingInputs {
  name: string
  email: string
...
export default function ReferenceForm() {
  ...
  const { data: applicant, error, isLoading } = useGetApplicationHeadQuery(membershipNumber || "", {skip: membershipNumber === undefined})

  const { register, control, handleSubmit, formState: { errors } } = useForm<ReferenceFormInputs>();
  const [ submitReference, { isLoading: isSubmitting, isSuccess: isSubmitted, error: submitError } ] = useSubmitReferenceMutation()

  const onSubmit: SubmitHandler<ReferenceFormInputs> = (data) => {
    submitReference({ membershipNumber, reference: data })
  }

  if(isLoading || applicant === undefined){
    return <Loading />
  } else if(isSubmitted){
    return <>
