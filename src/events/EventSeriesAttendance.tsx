...
export default function EventSeriesAttendance({eventSeriesId}: {eventSeriesId: string}) {
  ...
  if(allocationsIsLoading || membersIsLoading || allocations === undefined){
    return <Loading />
  ...
  let memberNames: {[membershipNumber: string]: string} = {};
  (members || []).forEach(m => memberNames[m.membershipNumber] = `${(m.preferredName || m.firstName)} ${m.surname}`);

  let membersAgg: {[membershipNumber: string]: {[allocationStatus in AllocationStatus]: number}} = {};
  ...
  let toolbar = () => {
    return <Toolbar>
      <ToolbarButton render={<ExportDataCsvButton data={rows} filename={`eventseries_${eventSeriesId}_allocations`} />} />
    </Toolbar>
  }
