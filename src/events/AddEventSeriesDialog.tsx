...
interface AddEventSeriesDialogProps {
  existing: string[]
  onClose: () => void
  show: boolean
}
...
export default function AddEventSeriesDialog(props: AddEventSeriesDialogProps) {
  const { register, handleSubmit, watch, reset, formState: { errors } } = useForm<EventSeriesFormValues>({mode: 'onTouched'});
  ...
  const onSubmit: SubmitHandler<EventSeriesFormValues> = (data) => {
    createEventSeries({
      eventSeriesId: id,
      body: data
    })
