import { ArrowForward, CurrencyPound, EmojiEvents, Event, MoreHoriz } from "@mui/icons-material"
import { Avatar, Box, Card, CardActions, CardContent, Dialog, DialogContent, DialogTitle, Grid, IconButton, Link, Typography } from "@mui/material"
import { useEffect, useState } from "react"
import { useDispatch } from "react-redux"
import { Link as RouterLink, useParams } from "react-router-dom"
import Error from "../common/Error"
import Loading from "../common/Loading"
import { useGetEventQuery, useRegisterForEventMutation } from "../redux/eventsApi"
...
import { ALLOCATION_ORDERING } from "../consts"
...
export default function ViewEvent(){
  ...
  // TODO: Change allocations
  // TODO: Menu for edit event, delete event
  // TODO: Descriptive text for criteria and weightings
  // TODO: Disable sign up if member doesn't meet criteria
  // TODO: Map?
  ...
  let spaces = "there is no limit on the number of attendees"
  if(event.attendanceLimit > 0){
    spaces = "there are "+event.attendanceLimit+" spaces available"
  }

  let criteria = null
  if(event.attendanceCriteria && event.attendanceCriteria.length > 0){
    criteria = <>
      <Typography variant="body1">The following attendance criteria applies to this event:</Typography>
      <ul>
        {event.attendanceCriteria.map((x, idx) => <li key={"criteria_"+idx}>x</li>)}
      </ul>
    </>
  }
  ...
  return <>
    <Grid container spacing={2}>
      <Grid item md={8}>
        ...
        <Typography variant="body1" gutterBottom>This event is open to all members of the QSWP, and {spaces}. 
      ...
      <Grid item md={4}>
        <Card elevation={3} sx={{marginBottom: '1rem'}}>
          <CardContent>
            ...
            <IconText icon={<ArrowForward />} marginBottom='1.5rem'>
        ...
        <Card elevation={3} className={"allocation_"+currentAllocation}>
          <CardContent>
            ...
            <AllocationWidget allocation={currentAllocation} verbose marginBottom='1.5rem'/>
            {allocations}
            {allocationCountText}
            
          </CardContent>
          {now.isAfter(registrationDate) || (currentAllocation && currentAllocation !== "REGISTERED") ? null : 
            <CardActions>
              <SubmitButton
    ...
    <Dialog onClose={() => setShowAllocations(false)} open={showAllocations} maxWidth="lg" fullWidth>
      <DialogTitle>Allocations for {event.name}</DialogTitle>
      <DialogContent>
