import { ArrowForward, Close, CurrencyPound, Delete, Edit, EmojiEvents, Event, MoreHoriz, Spa, Star } from "@mui/icons-material"
import { Avatar, Badge, Box, Button, Card, CardActions, CardContent, Grid, IconButton, Link, SpeedDial, SpeedDialAction, SpeedDialIcon, styled, Typography } from "@mui/material"
...
const BorderlessTooltip = styled((props: TooltipProps) => {
  ...
  return <Tooltip {...rest} classes={{ popper: props.className }} />
})(() => ({
...
export default function ViewEvent() {
  ...
  if (eventSeriesId === undefined || eventId === undefined) {
    return <Error>Event Series and/or Event ID not specified</Error>
  }
  ...
  fetchAuthSession().then(session => {
    const username = session.tokens?.accessToken.payload["username"]?.toString() || null;
  ...
  if (isLoading || membershipNumber === null || event === undefined) {
    return <Loading />
  } else if (error) {
  ...
  const allocationToOrder = (alloc: AllocationStatus) => {
    switch (alloc) {
      case ATTENDED:
        return 0;
      case ALLOCATED:
  ...
  if (event.allocations && event.allocations.length > 0) {
    ...
    allocations = <Box sx={{ display: 'flex', flexWrap: 'wrap', marginBottom: 1 }}>
      {sortedAllocations.map(a => {
        const isAllocated = a.allocation === ALLOCATED || a.allocation === ATTENDED;
        const isNewbie = isAllocated && a.experience === 0;
        const isPro = isAllocated && a.experience >= 3;
        const badgeIcon = isNewbie ? (
          <Spa data-testid="newbie-badge" sx={{ fontSize: 13, color: '#fff' }} />
        ) : isPro ? (
          <Star data-testid="pro-badge" sx={{ fontSize: 13, color: '#fff' }} />
        ) : null;
        const badgeTitle = isNewbie
          ? "First Timer (0 previous events)"
          : isPro
            ? `Pro (${a.experience} previous events)`
            : undefined;

        return (
          <BorderlessTooltip
            key={a.membershipNumber}
            title={
              <MemberCard
                membershipNumber={a.membershipNumber}
                name={(a.preferredName || a.firstName) ? `${a.preferredName || a.firstName} ${a.surname}` : undefined}
                experience={isAllocated ? a.experience : undefined}
          ...
          >
            <Badge
              ...
              sx={{
                margin: 0.5,
                '& .MuiBadge-badge': {
                  backgroundColor: (theme) => isNewbie ? "#007b41" : isPro ? theme.palette.secondary.main : undefined,
                  width: 20,
                  height: 20,
                  borderRadius: '50%',
                  minWidth: 20,
            ...
            >
              <Avatar
                className={"allocation_outline_" + a.allocation}
                sx={{ borderWidth: '2px', borderStyle: 'solid', height: '48px', width: '48px' }}
              >
      ...
      <Grid size={{ xs: 12, md: 4 }}>
        ...
        <Card elevation={3} className={"allocation_" + currentAllocation}>
          <CardContent>
            ...
            <AllocationWidget allocation={currentAllocation || null} verbose marginBottom={allocations !== null ? '1.5rem' : '0rem'} eligible={event.eligibility.eligible} suspended={memberSuspended} />
            {allocations}
            {allocationCountText}
