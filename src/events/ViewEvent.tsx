...
export default function ViewEvent(){
  ...
  if(event.allocations && event.allocations.length > 0){
    ...
    allocations = <Box sx={{display: 'flex', flexWrap: 'wrap', marginBottom: 1}}>
      {sortedAllocations.map(a => <BorderlessTooltip key={a.membershipNumber} title={<MemberCard membershipNumber={a.membershipNumber} name={(a.preferredName || a.firstName) ? `${a.preferredName || a.firstName} ${a.surname}` : undefined}/>}>
