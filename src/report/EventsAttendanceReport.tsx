      ...
      <Grid size={{xs: 6, md: 4, lg: 2}}>
        <Card variant="outlined">
          <CardContent>
            ...
            <Typography variant="h4" component="div">
              {Object.entries(report.counts.ATTENDED).reduce((acc, [k, v]) => acc + (parseInt(k) >= 2 ? v : 0), 0)}
            </Typography>
          </CardContent>
        </Card>
      </Grid>
      <Grid size={{xs: 6, md: 4, lg: 2}}>
        <Card variant="outlined">
          <CardContent>
            ...
            <Typography variant="h4" component="div">
              {Object.entries(report.counts.DROPPED_OUT).reduce((acc, [k, v]) => acc + (parseInt(k) >= 1 ? v : 0), 0)}
            </Typography>
          </CardContent>
        </Card>
      </Grid>
      <Grid size={{xs: 6, md: 4, lg: 2}}>
        <Card variant="outlined">
          <CardContent>
            ...
            <Typography variant="h4" component="div">
              {Object.entries(report.counts.NO_SHOW).reduce((acc, [k, v]) => acc + (parseInt(k) >= 1 ? v : 0), 0)}
            </Typography>
          </CardContent>
        </Card>
      </Grid>
      <Grid size={{xs: 6, md: 4, lg: 2}}>
        <Card variant="outlined">
          <CardContent>
            ...
            <Typography variant="h4" component="div">
              {Object.entries(report.counts.RESERVE).reduce((acc, [k, v]) => acc + (parseInt(k) >= 1 ? v : 0), 0)}
            </Typography>
          </CardContent>
        </Card>
      </Grid>
    ...
    <ResponsiveContainer width="100%" height={400}>
      <LineChart data={percentageAtLeast.map((value, idx) => { return { "days": idx, "pct": value*100 }})}>
        ...
        <Line dataKey="pct" fill="#a60c2b">
          <LabelList dataKey="pct" position="top"  formatter={lbl => !lbl || lbl == "0" ? "" : parseFloat(lbl.toString()).toFixed(2)} />
        </Line>
      </LineChart>
    </ResponsiveContainer>
  </>
