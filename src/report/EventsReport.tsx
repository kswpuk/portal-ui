...
export default function EventsReport() {
  ...
  const eventGrad = new Gradient()
    .setColorGradient("#ffffff", "#a60c2b")
    .setMidpoint(Math.max(eventMax + 1, 2))
    .getColors();
  const socialGrad = new Gradient()
    .setColorGradient("#ffffff", "#fdce72")
    .setMidpoint(Math.max(socialMax + 1, 3))
    .getColors();

  const pastEventCell = (date: string) => {
