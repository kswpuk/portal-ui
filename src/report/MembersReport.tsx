...
export default function MembersReport() {
  ...
  const time: ReportCount = {}
  Array.from(Array(timeMax).keys()).forEach(a => time[a] = report.counts.time[a] || 0)

  // Format labels
  const lblFormatter = (lbl: ReactNode) => {
    const l = lbl?.toString() ?? "00_100"
  ...
  return <>
    ...
    <BarChart data={(Object.keys(report.counts.age) as AgeCategories[]).map((key: AgeCategories) => { return { "ageGroup": key, "count": report.counts.age[key], "active": report.counts.ageActive[key], "inactive": report.counts.ageInactive[key] }})}>
      <CartesianGrid strokeDasharray="5 10" vertical={false} />
      <XAxis dataKey="ageGroup" tickFormatter={lblFormatter}/>
      <YAxis allowDecimals={false} domain={[0, 'dataMax']} />
      <Tooltip labelFormatter={lblFormatter} formatter={(value, name, props) => [value, (name?.toString() ?? "").charAt(0).toUpperCase() + (name?.toString() ?? "").slice(1)]} />
