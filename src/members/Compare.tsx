...
export default function Compare() {
  ...
  const [membershipNumbers, setMembershipNumbers] = useState<string[]>([])
  ...
  const onSubmit: SubmitHandler<CompareFormValues> = (data) => {
    setMembershipNumbers(data.membershipNumbers.split(/\n+/).map(n => parseInt(n.trim())).filter(n => n).map(n => n.toString()))
  }
  ...
  if(data && !isFetching && membershipNumbers.length > 0) {
    const Grey = (props: {children: ReactNode}) => <Box sx={{color: grey[500]}}><em>{props.children}</em></Box>
        ...
        sorting: {
          ...
          {field: "action", headerName: "Action", flex: 2, renderCell: params => {
            switch(params.row.action){
              ...
              case "NONE_GRACE":
                return <Grey>In grace period - no action required</Grey>
